...
from utils import padding_and_clip, get_onehot_vocab
...
class KRMBSeqReader(BaseReader):
    ...
    def _read_data(self, args):
        ...
        grouped_indices = self.log_data.groupby('user_id', sort=False).indices
        self.user_history = {uid: list(grouped_indices[uid]) for uid in self.users}
        ...
        self.item_meta, self.item_vocab = self._load_item_meta(
            args.item_meta_file, args.meta_file_sep, set(self.items))
    ...
    def _load_item_meta(self, path, sep, required_item_ids, chunksize=500_000):
        """Load all feature vocabularies, but retain metadata only for used items.

        Vocabulary order remains the first-occurrence order of the source CSV, as
        in ``get_onehot_vocab`` and ``get_multihot_vocab``.  Streaming avoids
        materializing the 32-million-row KuaiRand-27K table as Python objects.
        """
        onehot_features = self.selected_item_features[:-1]
        value_lists = {feature: [] for feature in onehot_features}
        seen_values = {feature: set() for feature in onehot_features}
        tag_values = []
        seen_tags = set()
        item_meta = {}
        usecols = ['video_id'] + self.selected_item_features

        for chunk in tqdm(pd.read_csv(path, sep=sep, usecols=usecols,
                                      dtype={'tag': str}, chunksize=chunksize),
                          desc="item metadata"):
            for feature in onehot_features:
                for value in chunk[feature].unique():
                    if value not in seen_values[feature]:
                        seen_values[feature].add(value)
                        value_lists[feature].append(value)
            for raw_tags in chunk['tag']:
                for tag_id in str(raw_tags).split(','):
                    if tag_id not in seen_tags:
                        seen_tags.add(tag_id)
                        tag_values.append(tag_id)

            selected = chunk[chunk['video_id'].isin(required_item_ids)]
            item_meta.update(selected.set_index('video_id').to_dict('index'))

        missing = required_item_ids.difference(item_meta)
        if missing:
            sample = list(missing)[:5]
            raise ValueError(f"Missing metadata for {len(missing)} items; examples: {sample}")

        vocab = {}
        for feature, values in value_lists.items():
            vocab[feature] = {}
            for index, value in enumerate(values):
                vector = np.zeros(len(values))
                vector[index] = 1
                vocab[feature][value] = vector
        vocab['tag'] = {}
        for index, tag_id in enumerate(tag_values):
            vector = np.zeros(len(tag_values))
            vector[index] = 1
            vocab['tag'][tag_id] = vector
        return item_meta, vocab
    ...
    def _sequence_holdout(self, args):
        ...
        for u in tqdm(self.users):
            row_ids = self.user_history[u]
            n_train = len(row_ids) - args.val_holdout_per_user - args.test_holdout_per_user
            if n_train < 0.6 * len(row_ids):
                continue
            data['train'].append(row_ids[:n_train])
            data['val'].append(row_ids[n_train:n_train+args.val_holdout_per_user])
            data['test'].append(row_ids[-args.test_holdout_per_user:])
